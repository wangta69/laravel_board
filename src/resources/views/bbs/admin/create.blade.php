...
@if (isset($cfg->id))
    <h2>게시판 수정</h2>
    {!! Form::open([
        'route' => ['bbs.admin.update', $cfg->id, 'urlParams='.$urlParams->enc],
...
<div class='form-group row'>
    ...
    <div class='col-sm-10'>
        ...
        <select id="roles-list" name="roles-list[]" class="form-control" multiple="multiple" style="width: 100%; display:none;" autocomplete="off">
            @foreach($roles as $role)
                <option @if(isset($cfg) && $cfg->roles_list->find($role->id)) selected="selected" @endif value="{{ $role->id }}">{{ $role->name }}</option>
            @endforeach
        </select>
    </div>
</div>
    ...
    <div class='col-sm-10'>
        <div class='row'>
            <div class='col-6 input-group'>
                ...
                <button type="button" class="btn btn-info btn-sm input-group-append" id="add-category" style="height: 33px;">생성</button>
            </div>
            <div class='col-6'>
                <table class='table table-borderless table-striped' id="category-list">

                    @forelse($cfg->category as $category)
...
<script>
var table_id = '{{$cfg->id}}';
var auth_list = '{{ isset($cfg->auth_list) ? $cfg->auth_list : 'login'}}';
        ...
        $.ajax({
            ...
            success: function(res){
                if (res.error === false) {
                    ...
                    addLine = addLine + '</td>';
                    addLine = addLine + '<td>';
                    addLine = addLine + '<button type="button" class="btn btn-danger btn-sm delete-category">삭제</button>';
                    addLine = addLine + '</td>';
                    addLine = addLine + '</tr>';
